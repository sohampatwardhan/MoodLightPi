...
impl MdnsResponder {
    ...
    /// Re-announces the current mDNS service, rebuilding the responder if its worker stopped.
    pub async fn refresh_records(&mut self) {
        if let Some(service) = &mut self.service {
            if service.announce() {
                debug!("refreshed mDNS records");
                return;
            }
        }

        warn!("mDNS responder is unavailable; recreating it");
        let (responder, task) = libmdns::Responder::with_default_handle().expect("creating mDNS responder");
