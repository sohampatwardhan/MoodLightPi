...
impl Server for IpServer {
    fn run_handle(&self) -> BoxFuture<Result<()>> {
        ...
        let mdns_refresh_responder = self.mdns_responder.clone();

        let handle = async move {
            let mdns_handle = mdns_responder.lock().await.run_handle();
            let mdns_refresh_handle = async move {
                let mut interval = tokio::time::interval(Duration::from_secs(30));
                loop {
                    interval.tick().await;
                    mdns_refresh_responder.lock().await.refresh_records().await;
                }
            };

            futures::try_join!(
                http_handle,
                mdns_handle.map(|_| Ok(())),
                mdns_refresh_handle.map(|_| Ok(()))
            )?;
