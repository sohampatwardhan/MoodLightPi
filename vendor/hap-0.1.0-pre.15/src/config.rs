...
const DEFAULT_SETUP_ID: &str = "MLPI";
...
impl Config {
    ...
    pub(crate) fn txt_records(&self) -> [String; 9] {
        [
            ...
            format!(
                "sh={}",
                setup_hash(DEFAULT_SETUP_ID, &self.device_id.to_hex_string())
            ),
        ]
    }
}
...
fn setup_hash(setup_id: &str, device_id: &str) -> String {
    ...
    digest.update(setup_id.as_bytes());
    digest.update(device_id.as_bytes());
    base64_four_bytes(&digest.finalize()[..4])
}

fn base64_four_bytes(bytes: &[u8]) -> String {
    ...
    let out = [
        ...
        TABLE[(b3 >> 2) as usize],
        TABLE[((b3 & 0b0000_0011) << 4) as usize],
        b'=',
        b'=',
    ];
    String::from_utf8(out.to_vec()).expect("base64 table is ascii")
}
