...
impl Service {
    pub fn announce(&mut self) -> bool {
        let svc = match self.services.read().unwrap().get_by_id(self.id) {
            Some(svc) => svc,
            None => return false,
        };

        self.commands.send_unsolicited(svc, DEFAULT_TTL, true)
    }
}
...
impl CommandSender {
    fn send(&mut self, cmd: Command) -> bool {
        let mut sent = false;
        self.0.retain(|tx| match tx.send(cmd.clone()) {
            Ok(()) => {
                sent = true;
                true
            }
        ...
        if !sent {
            warn!("mDNS responder channel closed");
        }
        sent
    }
