...
impl<AF: AddressFamily> FSM<AF> {
    ...
    fn add_ip_rr(&self, hostname: &Name, mut builder: AnswerBuilder, ttl: u32) -> AnswerBuilder {
        ...
        let mut selected = interfaces
            .iter()
            .filter(|iface| !iface.is_loopback())
            .filter(|iface| !is_usb_gadget_address(&iface.name, iface.ip()))
            .collect::<Vec<_>>();
...
fn is_usb_gadget_address(name: &str, ip: IpAddr) -> bool {
    ...
    match ip {
        IpAddr::V4(ip) => {
            ...
            octets[0] == 192 && octets[1] == 168 && octets[2] == 7
        }
        IpAddr::V6(_) => false,
    }
}
